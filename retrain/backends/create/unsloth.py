...
def create_unsloth(config: "TrainConfig") -> "TrainHelper":
    ...
    max_seq_length = backend_option_int(options, "max_seq_length")
    if max_seq_length <= 0:
    ...
    helper = UnslothTrainHelper(
        ...
        train_unsloth_fused_ce=str(options.get("train_unsloth_fused_ce", "auto")),
        ...
        gpu_memory_utilization=backend_option_float(
            options,
            "gpu_memory_utilization",
            0.5,
        ),
        float8_kv_cache=bool(options.get("float8_kv_cache", False)),
        max_lora_rank=backend_option_int(options, "max_lora_rank", 64),
        ...
        random_state=backend_option_int(options, "random_state", 3407),
