...
def render_commands(cli_name: str) -> list[str]:
    return [
        ...
        "",
        f"    {cli_name} resume-check <logdir> [--config config.toml] [--json]",
        "        Preflights a checkpoint/log directory before resuming.",
        "        Checks trainer_state.json, checkpoint payload, step bounds,",
        "        resume mode, and local SFT data recoverability.",
        "        --config PATH     checks target max_steps and SFT data pins.",
        "        --json            machine-readable JSON output.",
        "",
