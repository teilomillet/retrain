...
def collect_bounds_errors(config: TrainConfig, errors: list[str]) -> None:
    ...
    if config.clip_eps < 0:
        errors.append("clip_eps must be >= 0. Try: clip_eps = 0.2")
    if config.clip_eps_high < 0:
        errors.append("clip_eps_high must be >= 0. Try: clip_eps_high = 0.28")
    ...
    if config.policy_loss_mode not in ("standard", "kl_cov", "clip_cov"):
        errors.append("policy_loss_mode must be 'standard', 'kl_cov', or 'clip_cov'.")
    ...
    if config.adv_clip_max < 0:
        errors.append("adv_clip_max must be >= 0. Try: adv_clip_max = 5.0")
    if config.sft_batch_size < 0:
        errors.append("sft_batch_size must be >= 0. Try: sft_batch_size = 4")
    if config.sft_max_tokens < 0:
        errors.append("sft_max_tokens must be >= 0. Try: sft_max_tokens = 2048")
    if config.sft_lr < 0:
        errors.append("sft_lr must be >= 0. Try: sft_lr = 2e-5")
    ...
    if bool(config.sft_token_audit_path) != bool(config.sft_token_audit_sha256):
        errors.append(
    ...
    if config.sft_token_audit_path:
        ...
        if effective_sft_cap <= 0:
            errors.append(
                "sft_token_audit_path requires a positive effective SFT token cap."
            )
    ...
    if config.echo_weight < 0.0 or config.echo_weight > 1.0:
        errors.append("echo_weight must be in [0.0, 1.0]. Try: echo_weight = 0.05")
    if config.echo_enabled and config.echo_weight <= 0.0:
        errors.append("echo_enabled requires echo_weight > 0. Try: echo_weight = 0.05")
    ...
    if config.echo_loss_fn != "cross_entropy":
        errors.append("echo_loss_fn must be 'cross_entropy' for paper-faithful ECHO.")
    ...
    if config.echo_entropy_floor < 0.0:
        errors.append("echo_entropy_floor must be >= 0. Try: echo_entropy_floor = 0.0")
...
def _collect_optimizer_batch_errors(
    ...
    manifest_checksum = config.optimizer_batch_expected_manifest_sha256.strip().lower()
    ...
    if replay and not configured:
        errors.append(
            "trainer='optimizer_replay' requires [optimizer_batch].replay_path."
        )
    ...
    if config.optimizer_batch_capture and config.trainer != "retrain":
        errors.append("optimizer_batch.capture currently requires trainer='retrain'.")
    ...
    if not isinstance(allowed, list) or not all(
        ...
        if len(unique) != len(allowed):
            errors.append(
                "optimizer_batch.allow_config_differences must not contain duplicates."
            )
