...
def format_tree(tree: Tree) -> str:
    ...
    def _render(node_id: str, indent: str) -> None:
        if node_id in visited:
            ns = _get_node_state(tree, node_id)
            status = effective_status(node_id, tree, parent_map)
            icon = _icon(status, ns.outcome)
            lines.append(f"{indent}{icon} {node_id}  (see above)")
            return
        visited.add(node_id)
        ...
        lines.append(f"{indent}{icon} {node.id}  \"{node.label}\"{result_str}")

        child_indent = indent + "     "
        if node.on_success:
            refs = ", ".join(node.on_success)
            if len(node.on_success) == 1:
                lines.append(f"{child_indent}success -> ")
            else:
                lines.append(f"{child_indent}success ->")
            # Remove the incomplete line and render children inline
            lines.pop()
            for i, cid in enumerate(node.on_success):
                prefix = f"{child_indent}success -> " if i == 0 else f"{child_indent}         -> "
                # Render child inline for first level
                _render_inline(cid, prefix, child_indent + "              ")

        if node.on_failure:
            for i, cid in enumerate(node.on_failure):
                prefix = f"{child_indent}failure -> " if i == 0 else f"{child_indent}         -> "
                _render_inline(cid, prefix, child_indent + "              ")

    def _render_inline(node_id: str, prefix: str, sub_indent: str) -> None:
        if node_id in visited:
            ns = _get_node_state(tree, node_id)
            status = effective_status(node_id, tree, parent_map)
            icon = _icon(status, ns.outcome)
        ...
        if node.on_success:
            for i, cid in enumerate(node.on_success):
                p = f"{child_indent}success -> " if i == 0 else f"{child_indent}         -> "
                _render_inline(cid, p, child_indent + "              ")

        if node.on_failure:
            for i, cid in enumerate(node.on_failure):
                p = f"{child_indent}failure -> " if i == 0 else f"{child_indent}         -> "
                _render_inline(cid, p, child_indent + "              ")
...
def format_next(tree: Tree) -> str:
    """List all pending (actionable) nodes."""
    parent_map = _build_parent_map(tree)
    lines: list[str] = []
